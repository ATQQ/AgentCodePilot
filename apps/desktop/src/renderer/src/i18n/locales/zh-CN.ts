export default {
  ...
  settings: {
    ...
    gateway: {
      title: 'API 网关',
      desc: '对外提供 OpenAI / Anthropic 兼容接口，并按 Provider 转发到上游 API。可将 Claude Code、Claude Desktop、Codex 一键指向本机网关。',
      enable: '启用网关',
      runningAt: '运行中：{url}',
      stoppedDesc: '网关未运行',
      started: '网关已启动',
      ...
      takeoverTitle: '一键应用到客户端',
      takeoverDesc:
        '开启时会备份原配置并写入本机网关地址；关闭时自动还原。停用网关或退出应用时也会还原。',
      ...
      takeoverOff: '未接管',
      tabService: '服务',
      tabProviders: '上游 Provider',
      channelEnable: '启用此渠道接管',
      channelDesc: '开启后会备份该客户端原配置，并改写为指向本机网关；关闭时自动还原。',
      channelStatus: '当前状态',
      proxyTarget: '网关地址',
      backupAt: '备份时间',
      noBackup: '暂无备份',
      ...
      providersDesc:
        '全局配置 Provider：模型列表与默认模型只配一处；每种协议只需单独填写 baseUrl 与 API Key。各客户端通道只需选择使用哪种协议。',
      ...
      providerRequired: '请填写 ID、名称，并至少启用一个协议及 Base URL',
